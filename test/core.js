...
test('basic', async t => {
  ...
  const stats = await runWebpack({
    ...
    plugins: [new JSDelivrCdnWebpackPlugin()],
  });

  const files = stats.compilation.chunks.reduce(
    (files, x) => files.concat(x.files),
    [],
  );
  ...
  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/basic/app.js'),
  );
...
test('using production version', async t => {
  ...
  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/env-prod/app.js'),
  );
...
test('with mode=production', async t => {
  ...
  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/node-env-prod/app.js'),
  );
...
test('nested dependencies', async t => {
  await cleanDir(
    path.resolve(__dirname, './fixtures/output/nested-dependencies'),
  );
...
test('peerDependencies', async t => {
  await cleanDir(
    path.resolve(__dirname, './fixtures/output/peer-dependencies'),
  );
  ...
  t.true(files.includes('app.js'));
  t.true(
    files.includes('https://unpkg.com/@angular/core@4.2.4/bundles/core.umd.js'),
  );
...
test('load module without export', async t => {
  ...
  t.true(files.includes('app.js'));
  t.true(
    files.includes('https://unpkg.com/babel-polyfill@6.23.0/dist/polyfill.js'),
  );
});
...
test('exclude some modules', async t => {
  ...
  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/exclude/app.js'),
  );
...
test('only include some modules', async t => {
  ...
  t.false(
    files.includes('https://unpkg.com/react-dom@15.6.1/dist/react-dom.js'),
  );

  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/only/app.js'),
  );
...
test.serial(
  ...
  async t => {
    ...
    t.true(
      logs.includes(
        "❌ 'a' couldn't be found, please add it to https://github.com/mastilver/module-to-cdn/blob/master/modules.json",
      ),
    );
...
test('async loading', async t => {
  ...
  const outputs = await Promise.all(
    files.filter(x => !x.startsWith('https://unpkg.com')).map(async file => {
      return fs.readFile(
        path.resolve(__dirname, `./fixtures/output/async/${file}`),
      );
    }),
  );

  // NOTE: not inside t.false to prevent ava to display whole file in console
  const doesIncludeReact = outputs.some(output =>
    output.includes('THIS IS REACT!'),
  );
  t.false(doesIncludeReact);
});
...
test('when using multiple versions of a module, make sure the right version is used for each', async t => {
  await cleanDir(
    path.resolve(__dirname, './fixtures/output/multiple-versions'),
  );
  ...
  let output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/multiple-versions/app.js'),
  );
...
test('when using a custom resolver', async t => {
  ...
  let output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/custom-resolver/app.js'),
  );
  output = output.toString();
...
test('when one peerDependency fails, do not load from cdn', async t => {
  await cleanDir(
    path.resolve(__dirname, './fixtures/output/failing-peer-dependency'),
  );
  ...
  const stats = await runWebpack({
    ...
    output: {
      ...
      path: path.resolve(
        __dirname,
        './fixtures/output/failing-peer-dependency',
      ),
    },
  ...
  const doesIncludeAngular = output.includes(
...
test('when used with NamedModulesPlugin', async t => {
  ...
  const stats = await runWebpack({
    ...
    plugins: [new JSDelivrCdnWebpackPlugin(), new webpack.NamedModulesPlugin()],
  });
  ...
  const output = await fs.readFile(
    path.resolve(__dirname, './fixtures/output/named-modules/app.js'),
  );

  const doesHaveIncorrectRequire = output.includes(
    '__webpack_require__(undefined)',
  );
  t.false(doesHaveIncorrectRequire);

  const doesHaveCorrectReactRequire = output.includes(
